...
test('parseJsonlToMessages 坏行(JSON 解析失败)容错', () => {
  const jsonl = [
    '{not valid json',
    '',
    '{"type":"user","timestamp":"2026-07-03T08:00:00Z","message":{"role":"user","content":"hi"}}',
  ].join('\n');
...
test('parseJsonlToMessages timestamp < createdAt 过滤', () => {
  const jsonl = [
    '{"type":"user","timestamp":"2026-07-01T00:00:00Z","message":{"role":"user","content":"old"}}',
    '{"type":"user","timestamp":"2026-07-05T00:00:00Z","message":{"role":"user","content":"new"}}',
  ].join('\n');
...
test('parseJsonlToMessages 保留问题上下文标识 slug 不参与过滤(仅 basename 占位)', () => {
  // 当前实现不过滤 slug — 走时间过滤。slug 参数保留为 hook 备用。
  const jsonl = JSON.stringify({
    type: 'user',
    timestamp: '2026-07-03T08:00:00Z',
...
test('loadHistoryFromClaudeCode 注入临时 claudeHome,跨文件按 ts 升序合并 + 时间过滤', async () => {
  ...
  try {
    ...
    const sessionA = JSON.stringify({
      ...
      message: { role: 'user', content: 'A:user' },
    });
    const sessionB = JSON.stringify({
      ...
      message: {
        role: 'assistant',
        content: [{ type: 'text', text: 'B:assistant' }],
      },
    });
    const sessionC = JSON.stringify({
      type: 'user',
      timestamp: '2026-07-03T07:25:00Z', // 更早
      message: { role: 'user', content: 'C:user(更早)' },
    });
    ...
    // 期望按 ts 升序:C(07:25) → A(07:30) → B(07:35)
    assert.equal(out.length, 3);
    assert.equal(out[0]?.content, 'C:user(更早)');
    assert.equal(out[1]?.content, 'A:user');
    assert.equal(out[2]?.content, 'B:assistant');
...
test('parseJsonlToMessages 完整 user + assistant 往返示例', () => {
  const jsonl = [
    JSON.stringify({
      type: 'user',
      timestamp: '2026-07-03T08:00:00Z',
      message: { role: 'user', content: '帮忙看看 zhongche 的问题' },
    }),
    JSON.stringify({
      type: 'assistant',
    ...
    JSON.stringify({
      ...
      message: { role: 'user', content: '日志在 cwd 下' },
    }),
  ].join('\n');
  ...
  assert.equal(out.length, 3);
  assert.equal(out[0]?.role, 'user');
  assert.equal(out[0]?.content, '帮忙看看 zhongche 的问题');
