...
test('loadConfig parses 18 customers + 19 iterations from real config', async () => {
  const cfg = await loadConfig(GOOD_CONFIG);

  assert.equal(cfg.status, 'OK');
  assert.equal(cfg.data.customers.length, 18);
  assert.equal(cfg.data.iterations.length, 19);
