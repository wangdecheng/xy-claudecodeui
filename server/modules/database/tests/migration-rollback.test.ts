...
test('第 3 张表创建失败 → migrations_applied 事务回滚(完整性检查会发现该步骤缺失)', async () => {
  ...
  if (!result.ok) {
    assert.ok(result.missing.length >= 5, `应至少 5 个缺失 migration,实际 ${result.missing.length}`);
    assert.ok(result.missing.includes('001_add_sessions_kind_and_onsite_columns'));
...
test('verifyMigrations 在 corruption 下抛 MigrationCorruptionError(类形状正确)', async () => {
  await withIsolatedDatabase(() => {
    const db = getConnection();
    db.prepare(
      "UPDATE migrations_applied SET sha = 'tampered' WHERE name = '001_add_sessions_kind_and_onsite_columns'",
    ).run();
